...
if(env.BRANCH_NAME == "master" && lastCommitter != 'wealthwizardsjenkins') {

  stage 'Approve deploy to Blue'

    timeout(time: 14, unit: 'DAYS') {

      input "Continue deployment of $serviceName to Blue?"
  ...
  stage 'Deploy to Blue'
    ...
    node {
      ...
      try {
        ...
        k8s.safeDeploy(serviceName, 'blue', env.DOCKER_REGISTRY, buildVersion)
  ...
  stage 'Approve deploy to Red'

    timeout(time: 14, unit: 'DAYS') {

      input "Continue deployment of $serviceName to Red?"
  ...
  stage 'Deploy to Red'
    ...
    node {
      ...
      try {
        ...
        k8s.safeDeploy(serviceName, 'red', env.DOCKER_REGISTRY, buildVersion)
